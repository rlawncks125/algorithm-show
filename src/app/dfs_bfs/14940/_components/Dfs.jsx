...
function* dfsSolution(n, m, array, updateVisited) {
  ...
  function* dfs(x, y) {
    for (const [dx, dy] of direc) {
      ...
      if (
        ...
        // console.log(...visited);
        yield; // 여기서 yield 사용 가능
        yield* dfs(nx, ny); // 재귀 호출을 yield*로 변경
      }
    }
  }

  const [x, y] = queue.shift();
  yield* dfs(x, y); // dfs 호출을 yield*로 변경
}
...
export default function Answer({ n, m, array }) {
  ...
  const generateSolution = useCallback(
    () => dfsSolution(n, m, array, updateVisited),
    [n, m, array]
  );
